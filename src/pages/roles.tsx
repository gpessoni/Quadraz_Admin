...
export default function Equipaments() {
    ...
    const [roles, setRoles] = useState<Equipament[]>([])
    const [roleDialog, setRoleDialog] = useState<boolean>(false)
    const [role, setEquipament] = useState<Equipament>({
        id: null,
        name: "",
    })
    ...
    const saveEquipament = async () => {
        ...
        if (role.name.trim()) {
            let _equipament = [...roles]
            try {
                console.log("Payload enviado:", JSON.stringify(role))

                const isUpdating = !!role.id

                // Filtra os campos proibidos antes de enviar
                const { id, createdAt, updatedAt, ...equipamentData } = role

                const response = await fetch(isUpdating ? `/api/roles/update/${role.id}` : "/api/roles/create", {
                    method: isUpdating ? "PUT" : "POST",
                    headers: {
                        "Content-Type": "application/json",
                        Authorization: `Bearer ${getAuthToken()}`,
                    },
                    body: JSON.stringify(isUpdating ? equipamentData : { name: role.name }),
                })

                if (!response.ok) {
                    const errorData = await response.json()
                    const errorMessage = errorData.error || "Erro ao salvar Cargo"
                ...
                if (!isUpdating) {
                    ...
                    _equipament[_equipament.findIndex((g) => g.id === role.id)] = result
                }

                toast.current?.show({
                    severity: "success",
                    summary: "Sucesso",
                    detail: isUpdating ? "Cargo atualizado com sucesso" : "Cargo criado com sucesso",
                    life: 3000,
                })
                setRoles(_equipament)
                setRoleDialog(false)
                setEquipament({ id: null, name: "" })
            } catch (error) {
                console.error("Erro ao salvar Cargo:", error)
                const errorMessage = error instanceof Error ? error.message : "Erro desconhecido ao salvar Cargo"
                toast.current?.show({
                    severity: "error",
                    summary: "Erro",
                    detail: errorMessage,
    ...
    const editEquipament = (role: Equipament) => {
        setEquipament({ ...role })
        setRoleDialog(true)
    }

    const deleteEquipament = async (role: Equipament) => {
        try {
            ...
            if (response.ok) {
                let _equipament = roles.filter((val) => val.id !== role.id)
                setRoles(_equipament)
                toast.current?.show({
                    severity: "success",
                    summary: "Sucesso",
        ...
        <>
            ...
            <div className="card">
                ...
                >
                    ...
                    >
                        <label htmlFor="description">Tipo de Cargo</label>
                        <InputText
                            id="description"
                            value={role.name}
                            onChange={(e) => setEquipament({ ...role, name: e.target.value })}
                            style={{
                                marginTop: "10px",
                            }}
                            required
                            className={submitted && !role.name ? "p-invalid" : ""}
                        />
                    </div>
                </Dialog>
                <ConfirmDialog />
